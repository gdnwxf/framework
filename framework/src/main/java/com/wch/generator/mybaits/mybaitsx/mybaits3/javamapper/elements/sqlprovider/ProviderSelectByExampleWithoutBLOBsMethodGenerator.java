  ...
  public void addClassElements(TopLevelClass topLevelClass)
  {
    ...
    for (IntrospectedColumn introspectedColumn : getColumns()) {
      if (distinctCheck) {
        ...
        method.addBodyLine(String.format("SELECT_DISTINCT(\"%s\");", new Object[] { StringUtility.escapeStringForJava(MyBatis3FormattingUtilities.getSelectListPhrase(introspectedColumn)) }));

        method.addBodyLine("} else {");
        method.addBodyLine(String.format("SELECT(\"%s\");", new Object[] { StringUtility.escapeStringForJava(MyBatis3FormattingUtilities.getSelectListPhrase(introspectedColumn)) }));
    ...
    method.addBodyLine(String.format("FROM(\"%s\");", new Object[] { StringUtility.escapeStringForJava(this.introspectedTable.getAliasedFullyQualifiedTableNameAtRuntime()) }));
