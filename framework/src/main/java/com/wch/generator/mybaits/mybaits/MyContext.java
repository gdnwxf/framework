...
public class MyContext extends Context {
	...
	public void validate(List<String> errors) {
		...
		try {
			it = ObjectFactory.createIntrospectedTableForValidation((Context) this);
		} catch (Exception e) {
	...
	public XmlElement toXmlElement() {
		...
		if (this.commentGeneratorConfiguration != null) {
			xmlElement.addElement((Element) this.commentGeneratorConfiguration.toXmlElement());
		}
		if (this.jdbcConnectionConfiguration != null) {
			xmlElement.addElement((Element) this.jdbcConnectionConfiguration.toXmlElement());
		}
		if (this.javaTypeResolverConfiguration != null) {
			xmlElement.addElement((Element) this.javaTypeResolverConfiguration.toXmlElement());
		}
		if (this.javaModelGeneratorConfiguration != null) {
			xmlElement.addElement((Element) this.javaModelGeneratorConfiguration.toXmlElement());
		}
		if (this.sqlMapGeneratorConfiguration != null) {
			xmlElement.addElement((Element) this.sqlMapGeneratorConfiguration.toXmlElement());
		}
		if (this.javaClientGeneratorConfiguration != null) {
			xmlElement.addElement((Element) this.javaClientGeneratorConfiguration.toXmlElement());
		}
	...
	public CommentGenerator getCommentGenerator() {
		if (this.commentGenerator == null) {
			this.commentGenerator = ObjectFactory.createCommentGenerator((Context) this);
		}
	...
	public JavaFormatter getJavaFormatter() {
		if (this.javaFormatter == null) {
			this.javaFormatter = ObjectFactory.createJavaFormatter((Context) this);
		}
	...
	public XmlFormatter getXmlFormatter() {
		if (this.xmlFormatter == null) {
			this.xmlFormatter = ObjectFactory.createXmlFormatter((Context) this);
		}
	...
	public void introspectTables(ProgressCallback callback, List<String> warnings, Set<String> fullyQualifiedTableNames) throws SQLException, InterruptedException {
		...
		JavaTypeResolver javaTypeResolver = ObjectFactory.createJavaTypeResolver((Context) this, warnings);
		Connection connection = null;
		try {
			callback.startTask(Messages.getString((String) "Progress.0"));
	...
	public void generateFiles(ProgressCallback callback, List<GeneratedJavaFile> generatedJavaFiles, List<GeneratedXmlFile> generatedXmlFiles, List<String> warnings) throws InterruptedException {
		...
		for (PluginConfiguration pluginConfiguration : this.pluginConfigurations) {
			Plugin plugin = ObjectFactory.createPlugin((Context) this, (PluginConfiguration) pluginConfiguration);
	...
	private Connection getConnection() throws SQLException {
		Connection connection = ConnectionFactory.getInstance().getConnection(this.jdbcConnectionConfiguration);
		return connection;
	}
