...
public class SqlFilter {
	 ...
	 * @return
	 */
	public String getNoWhereHql() { 
		if (hql.indexOf(" where 1=1")>=0) {
			return hql.substring(11);
		}
		return hql.toString();
	}
	...
	public static void main(String[] args) throws ParseException {
		...
		sqlFilter.addFilter("QUERY_t#realName_S_LK", "dedee").addFilter("QUERY_t#proId_S_LK","dw232");;
		System.out.println(sqlFilter.getNoWhereHql());
		System.out.println(sqlFilter.getParams());
		System.out.println(DateUtils.parseDate("1989/06/28", new String[] { "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd HH:mm", "yyyy-MM-dd", "yyyy/MM/dd" }));
	}
	 ...
	 * @param params
	 */
	public SqlFilter addFilter(String name, String value) {
		if (name != null && value != null) {
			if (name.startsWith("QUERY_")) {// 如果有需要过滤的字段
				String[] filterParams = name.split("_");
				if (filterParams.length == 4) {
					String columnName = "";
					//当在java中传递的时候显示的用# 在get的网络请求下用@ 在极端的情况下用$
					if(filterParams[1].indexOf("#")>=0){
						columnName = filterParams[1].replaceAll("#", ".");// 要过滤的字段名称
					}else if(filterParams[1].indexOf("@")>=0){
						columnName = filterParams[1].replaceAll("@", ".");// 要过滤的字段名称
					}else if(filterParams[1].indexOf("$")>=0){
						columnName = filterParams[1].replaceAll("$", ".");// 要过滤的字段名称
					}
					...
					hql.append(" and " + columnName + " " + getSqlOperator(operator) + " :param" + no + " ");// 拼HQL
					params.put("param" + no, getObjValue(columnType, operator, value));// 添加参数
				}
				no++;
			}
		}
