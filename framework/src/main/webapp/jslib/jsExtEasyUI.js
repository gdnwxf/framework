...
sy.getParent = function (treeId,node){
	var parent = treeId.tree('getParent',node.target);
	if(!parent) return ;
	arguments.callee(treeId,parent);
};

sy.info = function(info){
	console.info('************组件信息start***********');
    console.info('组件的类型: '+(typeof info));
    if(info.length)
    console.info('组件的长度: '+info.length);
    console.info('组件的信息: ');
    console.info(info);
    console.info('************组件信息end*************');
};

sy.warn = function(){
	if((typeof arguments[0]) == 'function'||arguments[0]==window){
		  arguments[0].$.messager.alert('警告',arguments[1],'warning');  
	}else{
		 top.$.messager.alert('警告',arguments[0],'warning');
	}
};

sy.alert = function(){
	if((typeof arguments[0]) == 'function'||arguments[0]==window){
		  arguments[0].$.messager.alert('提示',arguments[1],'info');  
	}else{
		 top.$.messager.alert('提示',arguments[0],'info');
	}
};

sy.error = function(){
	if((typeof arguments[0]) == 'function'||arguments[0]==window){
		  arguments[0].$.messager.alert('错误',arguments[1],'error');  
	}else{
		 $.messager.alert('错误',arguments[0],'error');
	}
};
/**
 * 对$.messager.show方法的封装
 * 参数列表 sTop,sMsg,sTitle,iTimeout
 */
sy.show = function(msg){
		top.$.messager.show({
		title : '提示消息',
		msg : msg,
		timeout : 2000,
 ...
 * 获取当前选中的row
 */
sy.slectRow =  function (idName,type){
	var obj = idName;
	if((typeof idName) == 'string'){
	    obj = $('#'+idName);
	}
	if(type=='treegrid'){
		return obj.treegrid('getSelected');
	}else if(type=='tree'){
		return obj.tree('getSelected');
	}else if(type=='datagrid'){
		return obj.datagrid('getSelected');
	}else if(type=='combobox'){
		return obj.combobox('getSelected');
	}else if(type=='combotree'){
		var t = obj.combotree('tree');	// 获取树对象
		return  t.tree('getSelected');
	}else if(type=='combogrid'){
		var g = obj.combogrid('grid');	// 获取数据表格对象
		return g.datagrid('getSelected');
	}else if(type=='tabs'){
		 return obj.tabs('getSelected');  //获取tabs选中的
	}else {
		return '未找到';
	}
};
...
sy.UUID = function() {
	return (sy.random4() + sy.random4() + sy.random4() + sy.random4()
			+ sy.random4() + sy.random4() + sy.random4() + sy.random4());
};
		...
		if (opt.parentField) {
			...
			for (i = 0, l = data.length; i < l; i++) {
				...
				if (tmpMap[data[i][parentField]] && data[i][idField] != data[i][parentField]) {
					if (!tmpMap[data[i][parentField]]['children']) //父节点的子节点不存在
...
$.extend($.fn.treegrid.defaults, {
	loadFilter : function(data1, parentId) {
		var data;
		if(data1.rows){ data = data1.rows; }else { data = data1; }
		...
		if (opt.parentField) {
			...
			if(data1.rows) {data1.rows = treeData; return data1;}else{ return treeData;} 
			//return treeData;
		}
		if(data1.rows) {data1.rows = data; return data1;}else{ return data;} 
		//return data;
	}
});
