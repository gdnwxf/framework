 ...
 */
sy.getPath = function(){
	var serverPath = window.document.location.href;
	var pathName = window.document.location.pathname;
	var pos = serverPath.indexOf(pathName);
	var localhostPath = serverPath.substring(0,pos);
...
/**
 * 如果name == '' 则由外面进行name的封装 Object 
 * 如果obj.length == 0 就返回一个空的对象 
 * item 为 obj 需要封装的属性数组
 */
sy.getObjectArrayString = function(obj,name,item){
	 if(obj.length==0){
		 return {};
	 }
	 var x = new Array();
 ...
 * 无特殊用途
 */
sy.serialize$Object= function(obj,name){
	var o = {};
	for (var i = 0; i < obj.length; i++) {
		o[name] = obj[i];
	}
